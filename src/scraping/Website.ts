...
async function giveWebsiteInfo(url:string): Promise<ScrapedData | { error: string }> {
  let browser = null;
  let page = null;
  const content = [];
      ...
      elements.forEach((el) => {
        ...
        if (el.tagName === "BLOCKQUOTE") {
          ...
          if (codeElement instanceof HTMLElement) {
            ...
            "><${codeElement.tagName} class="${customClass}" style="${customStyles}">${el.textContent}</${codeElement.tagName}></blockquote>`
            );
          }else{
            result.push(`
              <blockquote class="${customClass}" style="${customStyles};
                ...
                margin-bottom: 1.6em;
                padding-inline-start: 1em;
              ">${el.textContent}</blockquote>`);
          }
          }
        } else if (el.tagName === "P" && !isInsideBlockquote) {
          // Only process <p> if it's NOT inside a <blockquote>
          result.push(
            `<p class="${customClass}" style="${customStyles}">${el.textContent}</p>`
          );
        }
        // Process each element based on its tag name
        else if (el.tagName === "H1") {
          result.push(
            `<h1 class="${customClass}" style="${customStyles}">${el.textContent}</h1>`
          );
        } else if (el.tagName === "H2") {
          result.push(
            `<h2 class="${customClass}" style="${customStyles}">${el.textContent}</h2>`
          );
        } else if (el.tagName === "H3") {
          result.push(
            `<h3 class="${customClass}" style="${customStyles}">${el.textContent}</h3>`
          );
        } else if (el.tagName === "H4") {
          result.push(
            `<h4 class="${customClass}" style="${customStyles}">${el.textContent}</h4>`
          );
        } else if (el.tagName === "H5") {
          result.push(
            `<h5 class="${customClass}" style="${customStyles}">${el.textContent}</h5>`
          );
        } else if (el.tagName === "H6") {
          result.push(
            `<h6 class="${customClass}" style="${customStyles}">${el.textContent}</h6>`
          );
        } else if (el.tagName === "PRE") {
          let codeElement = el.querySelector("code");
          if (codeElement) {
            result.push(
              `<pre style="color:#333; padding:15px; background-color:#f5f5f5; overflow-x: auto; border-radius: 5px; font-family: monospace; font-size: 14px; box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.1);"><${codeElement.tagName} class="language-shell">${codeElement.textContent}</${codeElement.tagName}></pre>`
            );
          } else {
            result.push(
              `<pre style="color:#333; padding:15px; background-color:#f5f5f5; overflow-x: auto; border-radius: 5px; font-family: monospace; font-size: 14px; box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.1);">${el.textContent}</pre>`
            );
          }
        } else if (el.tagName === "A") {
          result.push(
            `<a class="${customClass}" style="${customStyles}" href="${(el as HTMLAnchorElement ).href}" target="_blank">${el.textContent}</a>`
          );
        }  else if (el.tagName === "HEADER") {
          result.push(
            `<header class="${customClass}" style="${customStyles}">${el.textContent}</header>`
          );
        } else if (el.tagName === "IMG") {
          result.push(
            `<img src="${(el as HTMLImageElement).src}" alt="image" style="max-width:100%"/>`
          );
          ...
          let listContent = Array.from(listItems)
            .map(
              (li) =>
                `<li class="${customClass}" style="${customStyles}">${li.textContent}</li>`
            )
            .join("");
          result.push(
